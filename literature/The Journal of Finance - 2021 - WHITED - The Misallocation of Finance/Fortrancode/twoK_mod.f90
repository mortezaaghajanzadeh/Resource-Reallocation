module datatype      ! This module defines double precision and makes all of the data structures
    INTEGER, PARAMETER     :: dp = SELECTED_REAL_KIND(14, 60)

    type :: policies                    ! policy matrices
         sequence
         real(dp) :: k                    ! capital
         real(dp) :: n                    ! intangibles
         real(dp) :: b                    ! debt
         real(dp) :: d                    ! dividends
         real(dp) :: e                    ! equity issuance
         real(dp) :: l                    ! labor
         real(dp) :: y                    ! output
         real(dp) :: i                    ! investment
         real(dp) :: r                    ! intangible investment
    end type policies

    type :: weights                       ! interpolation weights
         sequence
         real(dp) :: k                    ! capital
         real(dp) :: n                    ! intangibles
         real(dp) :: b                    ! debt
    ...
    type :: ipolicies                   ! policy index matrices
         sequence
         integer :: k
         integer :: n
         integer :: b
    ...
    type :: grids                       ! grids
         ...
         real(dp) :: n
         real(dp) :: b
         real(dp) :: z                    ! profit shock
         real(dp) :: e                    ! some other optional shock
    end type grids

    type :: gridinterps                   ! interpolation weights and endpoints for the VFI
         sequence
         integer :: lo                    ! weights for all possible policies
         integer :: hi
         real(dp) :: w
    end type gridinterps
...
implicit none
    ...
    integer, parameter  :: nz  = 11!15!11  ! Profitability grid
    integer, parameter  :: nk  = 15!19!15  ! Capital stock grid
    integer, parameter  :: nn  = 7      ! Intangible intensity grid
    integer, parameter  :: nb  = 9!11!9   ! Leverage grid
    integer, parameter  :: gmax = 101   ! This is a weakness in the code. You have to hand code in the max grid dimension.

    integer, parameter  :: npk = 61!91!61  ! Capital stock policy grid!
    integer, parameter  :: npn = 21     ! Intangible stock policy grid
    integer, parameter  :: npb = 61!91!61  ! Debt policy grid
    integer, parameter  :: egmax = 201  ! This is a weakness in the code. You have to hand code in the max grid dimension.

    integer, parameter  :: pad = 5      ! the padding around the old optimal point for searching

    integer, parameter  :: nsz = 59

    ! Number of parameters and number of moments
    integer, parameter  :: allnop=13      ! Make sure that this is compatible with the compstat setting.    This is generality in case you want to zero out some parameters.
    integer, parameter  :: nop=13
    integer, parameter  :: numv = 7
    integer, parameter  :: nmom = 24
    integer, parameter  :: numinfl = 24
    integer, parameter  :: nagg = 4

    integer, parameter  :: mewave   = 1     !Monte Carlo or exact aggregates
    integer, parameter  :: compstat = 0     !This prints out little files.
    integer, parameter  :: verbose  = 0     !Intermediate screen output


    ! This controls whether you use the simulation to calculate a steady state.
    integer, parameter :: steadystate = 0

    ! Specify the parameters in the simulations
    integer, parameter  :: nFirms = 2000
    integer, parameter  :: nYears = 250  !80
    integer, parameter  :: burn_in =200
    ...
    integer, parameter  :: printlittle=0
    ...
    integer, parameter  :: complicated = 0
    ...
    character (len=5), parameter :: period = " ", siz = " ", estype=" "
    integer :: nfcnev, errcode ! This is the number of simulated anneal evalutations.
...
! Stuff for SMM that is a pain to pass around

integer :: ooseed(1)
real(dp) :: datamoms(nmom)
real(dp) :: weight(nmom,nmom)
real(dp) :: iweight(nmom,nmom)
integer :: gendata, icount=0
real(dp), allocatable :: simdata(:,:,:)
real(dp) :: share=0.5_dp
real(dp) :: rf=0.02_dp, tau = 0.35_dp
real(dp) :: aggY, aggC, aggI, aggN, aggL, aggW, aggKay, aggR, TFP
...
implicit none
    ...
    !=======================note: this is an abbreviated moment list==============================================================
      integer :: ayeaye
      integer, parameter,dimension(nmom)  :: pickout = (/(ayeaye, ayeaye=1,nmom )/)

      character (len=11),parameter,dimension(allnop) :: pname = (/ &
      'alpha          ',   &       !   capital share
      'beta           ',   &       !   intangible share
      'rho            ',   &       !   serial correlation
      'sigma          ',   &       !   standard deviation
      'delta          ',   &       !   depreciation
      'inlta          ',   &       !   intangible depreciation
      'adjK           ',   &       !   capital adjustment costs
      'adjN           ',   &       !   intangible adjustment costs
      'xi             ',   &       !   collateral
      'lambda         ',   &       !   equity issuance cost
      'dishcost       ',   &       !   debt issuance cost
      'divsmooth      ',   &       !   dividend smoothing
      'wage           ' /)         !   wage
      integer, parameter, dimension(nop) :: pickparam = (/ &
      1  ,   &
      2  ,   &
      3  ,   &
      4  ,   &
    ...
    character(len=19),parameter,dimension(numinfl) :: amomname = (/     &
     'mu_invest          ', &
     'mu_intang          ', &
     'mu_lev             ', &
     'mu_opinc           ', &
...
implicit none
   ...
   real(dp), intent(in)  :: omega(rows,cols)
   real(dp), intent(out) :: iomega(rows,cols)
   real(dp) :: u(rows,cols), v(rows,cols), s(rows), e(rows), eyes(rows,cols), omg(rows,cols)
   integer :: info,ii,jj
