...
\begin{equation}
    \begin{split}
        \epsilon \equiv& \frac{E}{\nu_{s}(PY)^{\frac{\sigma_s}{\sigma_s - 1}}}  = \frac{\tilde{A}}{\hat{A}}\left(
            \alpha_s {{z^k}}^{\gamma_s - 1} + (1-\alpha_s)
        \right) ^ {\frac{\gamma_s}{1-\gamma_s}} {{z^l}}^{1 - \beta} \\
        \Rightarrow & \ln \epsilon = \ln \tilde{A} - \ln \hat{A} + \frac{\gamma_s}{1-\gamma_s} \ln \left(
            \alpha_s {{z^k}}^{\gamma_s - 1} + (1-\alpha_s)
        \right) + (1-\beta) \ln {{z^l}}\\
        \Rightarrow & \frac{\partial \epsilon/\partial \tau_E}{\epsilon} = 
    \end{split}
\end{equation}
